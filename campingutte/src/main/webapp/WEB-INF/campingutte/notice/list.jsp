		...
		<table class="table table-border table-list">
			...
			<c:forEach var="dto" items="${list}">
				<tr>
					<td>${dto.listNum}</td>
					<td class="left">
						<a href="${articleUrl}&notiNo=${dto.notiNo}">${dto.notiSubject}</a>
					</td>
					<td>${dto.memberName}</td>
					<td>${dto.notiDate}</td>
					<td>${dto.notiHitCount}</td>
				</tr>
			</c:forEach>
		</table>
				...
				<td align="center">
					<form name="searchForm" action="${pageContext.request.contextPath}/notice/list.do" method="post">
						<select name="condition" class="selectField">
							...
							<option value="notiDate"  ${condition=="notiDate"?"selected='selected'":"" }>등록일</option>
							<option value="notiSubject"  ${condition=="notiSubject"?"selected='selected'":"" }>제목</option>
							<option value="notiContent"  ${condition=="notiContent"?"selected='selected'":"" }>내용</option>
						</select>
